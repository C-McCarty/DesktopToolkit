...
class Sidebar(tk.Frame):
    ...
    def _build_scroll(self):
        ...
        # The wheel is bound to the canvas here and, on every rebuild, to all the
        # per-monitor widgets (see _bind_wheel_recursive) so scrolling works over
        # any part of the panel — not just the empty canvas space. The handler
        # ignores the event unless the content actually overflows the viewport.
    ...
    def _on_mousewheel(self, e):
        # Only scroll when the monitor list is taller than the visible viewport;
        # with a short list there is nothing to scroll.
        if self._frame.winfo_reqheight() <= self._inner_canvas.winfo_height():
            return
        self._inner_canvas.yview_scroll(int(-1 * (e.delta / 120)), 'units')

    def _bind_wheel_recursive(self, widget):
        """Bind wheel scrolling to a widget and all of its descendants so the
        panel scrolls regardless of which child the pointer is over."""
    ...
    def load_monitors(self, monitors):
        ...
        # Re-bind the wheel across the freshly built widgets so scrolling works
        # anywhere over the panel.
        self._bind_wheel_recursive(self._frame)
